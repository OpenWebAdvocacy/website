...
Remote tab in-app browsers such as Android Custom Tabs are a potentially ideal solution for most users. Android Custom Tabs by default invokes the users default browser. This is an interesting middle ground where the user (and the app) can benefit from not leaving the app context while still respecting the users choice of default browser and preserving the users privacy. However, currently it is possible for the hosting Native App to lock Android Custom Tabs to a particular browser and override the users choice of default browser. This ability needs to be removed.

While iOS's remote tab in-app browser SFSafariViewController does protect the users privacy, it is locked to Safari (or more specifically the WkWebView) and thus overrides the users choice of default browser. 
