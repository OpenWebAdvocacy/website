...
As OWA anticipated, Apple is proposing a new [Web Browser Engine Entitlement](https://developer.apple.com/support/alternative-browser-engines/#web-entitlement) that allows browser apps some of the extended privileges any modern, safe browser requires. Apple is also proposing a list of requirements browser vendors must meet to be eligible.


Some deal with functional aspects of web content rendering, ensuring that browser engines are minimally standards compliant. Others deal with security aspects of web browsing, and yet others impose specific constraints related (in Apple’s view) to privacy. 

The security requirements are the least objectionable, and it's worth noting that many major browsers are compliant with the [NIST guidelines for critical software supply chains.](https://www.nist.gov/itl/executive-order-improving-nations-cybersecurity/security-measures-eo-critical-software-use-2) and thus and presumably with Apple’s looser restatements of those processes. OWA's stance is that it is reasonable (and allowed by the act) for Apple to set a minimum bar for browser security on iOS on the proviso that the rules are reasonable, proportional and do not preclude smaller browser vendors that take security seriously from competing on iOS with their own engine.


It’s worth noting that the DMA only grants Apple the right to “strictly necessary and proportionate, measures to ensure that third-party software applications or software application stores do not endanger the integrity of the hardware or operating system”, with the burden of proof as to their necessity and proportionality being upon Apple. We concur with the intent of the act and believe other aspects such as performance and functionality should be left to competition.

Apple requires that browsers secure a “90% from Web Platform Tests”, but this is nonsensical and requires immediate clarification. The [Web Platform Tests project](https://wpt.fyi/about) is a comprehensive corpus of tests for browser engines, but there is no generally accepted set out of which 90% can be measured, as each engine project enables or disables test groups based on features that are minimally implemented. An engine that implements very few features could easily maintain a 90% pass rate _of the features it provides_, while still falling well short of the stated goal of broad standards conformance. The test corpus also changes between the desktop and mobile versions of browsers. Without clarification about which group of tests the 90% will be calculated from, developers will live under a cloud of doubt. It’s also problematic that Apple’s unclear language also potentially demands pass-rate statistics that cannot be produced today, as no set of WPT tests are run on iOS for any browser, using any engine (although Android browsers are tested) -- including Safari. 

The nominal privacy requirements create deep concerns. Instead of enabling market competition to improve privacy, Apple is demanding specific technical mechanisms that its own products do not abide (e.g., “boostrap” cookies for Web App installation to the homescreen). It also creates vague, unenforceable terms about “informed user activation” that read like a blank check for Apple’s reviewers to reject browsers based on vibes, rather than analysis. Even with a very brief investigation, we found Apple [does not yet follow these guidelines](https://webkit.org/blog/8311/intelligent-tracking-prevention-2-0/#:~:text=Temporary%20Compatibility%20Fix%3A%20Automatic%20Storage%20Access%20for%20Popups). Apple’s own track record regarding many of the issues it is performatively demanding conformance with is poor, and OWA believes that much of this language needs to be trimmed and made more objective. The primary concern here is that Apple may use this rule as a tool to deny third party browsers the ability to implement important Web App functionality with the aim of keeping that functionality exclusive to Native Apps sold via their App Store.

Further requirements related to code used by alternate browser engines is even more worrying. Apple is demanding specific iOS APIs to be used for features such as text input, scrolling, dragging, and contextual menus. Although this may seem appropriate to provide iOS users with a uniform experience, consistency can be achieved in many ways. Indeed, most browser engines work extremely hard to integrate well into their host OSes and provide the best possible experience. Hard requirements in this area to use specific APIs seem hard to justify given macOS Safari’s long history of private API abuse for competitive advantage.
...
Indeed, some of these APIs have deep implications for competing engine codebases. It is thus also likely to dramatically increase the complexity of the engine itself and make further developments and its maintenance harder for the browsers’ teams. We also note that when Apple has [shipped Safari on non-Apple platforms](https://www.apple.com/newsroom/2007/06/11Apple-Introduces-Safari-for-Windows/), it was not subject to similar restrictions and would be highly unlikely to accept them for iOS Safari itself.
Adoption of such APIs should be optional, leaving vendors free to incorporate them as they see fit, and at their own pace. Otherwise, some browsers might very well be incapable of bringing their engine to iOS.
...
Alternate browsers should also be able to handle Push Notifications for Web Apps, as well as badging when they are installed. Again, there is no mention of APIs allowing the implementation of such features in Apple’s documentation.
...
Most aggressively, Apple proposes only to allow competitors to ship their own engines inside the EU. This is an attempt to add poison pills for browser makers, driving up costs to maintain multiple iOS browsers (one for Europe, one for the rest of the world), and to impose costs that Apple does not bear and would never stand for if imposed by a competitor. OWA expected some amount of awful-but-lawful malicious compliance, but continued restrictions on engine choice outside the EU are egregious. 
...
&mdash; [Apple Documentation](https://developer.apple.com/support/alternative-browser-engines/)
...
a) Browser vendors must ship a separate app using their own browser engine.
b) Browser vendors can seamlessly swap over their existing users to the updated browser now using its own browser engine.
...
It’s worth stressing here that no one on our team is a lawyer. But to our un-lawyerly eyes a number of clauses in Apple’s contract jumped out at us as not only unfair but also in direct violation of the DMA. 

It’s also worth highlighting that to our knowledge a specific contract to be “allowed” to use your own browser engine is unique to Apple in the history of both the internet and computing.
...
&mdash; [Apple’s New Browser Engine Contract](https://developer.apple.com/contact/request/download/web_browser_engine.pdf)
...
It’s worth noting another clause states that browsers as part of this agreement must also abide by the Apple App Store Guidelines (guidelines being a delightfully Orwellian phrase when they are clearly rules). Apple does not have the best reputation as to being a fair judge of what counts as a violation of its app store rules.
...
Apple's proposal also included plans for how they would allow third party native app stores to compete with their own on iOS. This is directly compelled by the act.

While allowing third party native app stores is not part of the fight of our organization, it is relevant in so far as browsers, being free products, would like to list on every major app store available on iOS. Apple's proposal seems designed, almost spitefully so, to prevent any free app from ever even considering listing on a third party native app store on iOS.
...
The pricing difference between the contracts is really worth delving into, particularly for free apps like browsers. If you choose the second contract, Apple will waive some app store fees but will add a new fee called "Core Technology Fee'' which basically boils down to a 50c charge per a user per a year.
...
[Apple Documentation](https://developer.apple.com/support/core-technology-fee/)
...
[Digital Markets Act](https://eur-lex.europa.eu/legal-content/EN/TXT/?toc=OJ%3AL%3A2022%3A265%3ATOC&uri=uriserv%3AOJ.L_.2022.265.01.0001.01.ENG)
(emphasis added)
...
Apple’s policy does not sound fair, reasonable or non-discriminatory either to free apps (such as browsers) or to third party app stores.


If any of Apple's proposal sounds ludicrous to you, you are not alone. We are scratching our heads as to how Apple's lawyers thought this would possibly be compliant with the DMA and the degree to which this would alienate native app developers. Indeed their very willingness to pursue this despite, presumably, being aware how wildly unpopular it would be is a striking example of their market power gained by being able to block users from installing any particular app on the user's own devices.
...
Even on traditionally pro-apple forums the response to [Apple's browser competition proposal](https://www.macrumors.com/2024/01/26/mozilla-on-apple-eu-browser-engine-change/) in the comments is extremely negative. Apple is losing not only this battle but also their reputation. With each market investigation or act passed, this behavior is forced into the open and Apple must fight to defend their "right" to block third party browsers and Web Apps from competing with their own products and app store.
...
&mdash; [EU Industry Chief Thierry Breton](https://www.reuters.com/technology/apple-faces-strong-action-if-app-store-changes-fall-short-eus-breton-says-2024-01-26/)
...
&mdash; [Thomas Clayburn - The Register](https://www.theregister.com/2024/01/27/apple_europe_ios_analysis/)
